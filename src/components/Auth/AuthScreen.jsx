...
export function AuthScreen({ onAuth }) {
  ...
  async function handle() {
    ...
    try {
      if (mode === "login") {
        const authData = await authRequest("/token?grant_type=password", { email, password: pass });
        ...
        setMsg("Conta criada! Verifique seu e-mail e faça login.");
        setMode("login");
      } else {
    ...
    <div style={{ minHeight: "100vh", background: C.slate, display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem 1rem" }}>
      <div style={{ width: "100%", maxWidth: 420, animation: "fadeUp .5s ease" }}>
        ...
        <div style={{ background: C.white, border: "1px solid " + C.border, borderRadius: 20, padding: "2rem", boxShadow: "0 16px 48px rgba(30,58,138,.1)" }}>
          ...
          {msg && (
            <div style={{ background: "#ECFDF5", border: "1px solid #A7F3D0", borderRadius: 10, padding: "10px 14px", fontSize: 13, color: C.greenD, marginBottom: 14 }}>
              {msg}
            </div>
          )}
          ...
          {mode === "login" && (
            <button onClick={() => { setMode("forgot"); setErr(""); }} style={{ display: "block", margin: "12px auto 0", background: "none", border: "none", cursor: "pointer", fontSize: 12, color: C.grayD }}>
              Esqueci minha senha
            </button>
          )}
          
          {mode === "forgot" && (
            <button onClick={() => { setMode("login"); setErr(""); }} style={{ display: "block", margin: "12px auto 0", background: "none", border: "none", cursor: "pointer", fontSize: 12, color: C.grayD }}>
              ← Voltar
            </button>
          )}
        </div>
